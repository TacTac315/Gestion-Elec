...
//Variables et paramètres liées à la commande PWM et son réglage
...
extern uint16_t DutyDefaultGD3000; //rapport cyclique par défaut (en nb de clock submodule)
extern uint32_t DeadTime_PWM; ////valeur du deadtime (réglé au niveau du module FlexPWM), donné en nb de périodes
		//d'horloge MOTC (fréq = 40 MHz --> période = 25 ns). Même valeur pour fronts montants et descendants
extern uint8_t Align_PWM; //réglage de l'alignement de la PWM : CENTER_ALIGN ou EDGE_ALIGN


//variables liées à la mesure des courants de phase
...
uint32_t Delay_Trig_ADC_us; //délai en µs entre le trigger issu de la PWM (dépassement VAL1 pour déclencher l'acquisition
...
//indique si on utilise la Space Vector Modulation (SVM)
uint8_t SVM_Activate;
 ...
 * La variable DeadTime donne la valeur du deadtime, définie par :
 *  DeadTime*Freq_MOTC (40 MHz). Même valeur sur fronts montants et descendants
 * La variable Align_PWM règle l'alignement de la PWM : deux modes d'alignement sont supportés :
 * EDGE_ALIGN et CENTER_ALIGN.
 */
/*****************************************************************************/
void init_Cmd_Moteur_Variables(uint8_t Moteur_Reel_Ou_Virtuel, uint8_t SVM_On_or_Off, uint32_t DeadTime, uint32_t Align) {
...
//	DeadTime_PWM  = 100; //valeur du deadtime = DeadTime_PWM*Freq_MOTC (40 MHz) ts et descendants
//	Align_PWM = CENTER_ALIGN; //EDGE_ALIGN ou CENTER_ALIGN
	DeadTime_PWM  = DeadTime;
	Align_PWM = Align;

	//paramètres pour la mesure de courant
	Delay_Trig_ADC_us = 0; //délai entre trigger PWM et acquisition du courant low side par l'ADC (en µs)
	...
	SVM_Activate = SVM_On_or_Off;
}

/*****************************************************************************/
/* raz_Cmd_Moteur_Reel_Variables                                                  */
/* Description: remise à zéro des variables requises pour le contrôle de type U/f
 * A utiliser uniquement après un arrêt du moteur, jamais quand le moteur tourne.
 */
/*****************************************************************************/
void raz_Cmd_Moteur_Variables(void) {
	Consigne_Vitesse = 0;
	Commande_Vitesse = 0;
	...
	//ors du prochain démarrage, le programme entrera en mode de calibration de l'offset des ADC
	Mode_Sequence_Moteur = MODE_FCT_CAL;
	Cptr_Mes_Cal = 0;
...
SWLIBS_3Syst_F16 Exec_Fast_Loop(SWLIBS_2Syst_FLT Udq_ref) {
	...
	//mapping Ualpha et Ubeta --> rapports cycliques associés à Ua, Ub et Uc
	if (SVM_Activate == SVM_ON) {
		//la fonction SvmStd fournit directement des rapports cycliques compris entre 0 et 1.
		SVMSector = GMCLIB_SvmStd_FLT(&PWM_abc_Out,&AlphaBetaOut);
	}
	else {
...
tFloat U_F_LUT(tFloat Vitesse_in) {
	...
	if ((Vitesse_in <= 1500) & (Vitesse_in >= -1500)) {
		if ((Vitesse_in > 500) | (Vitesse_in < -500)) {
			Uq_ref = Vitesse_in*0.006;
		}
		else if (Vitesse_in > 0) {
			Uq_ref = 3;
		}
		else if (Vitesse_in < 0) {
			Uq_ref = -3;
...
void ADC0_EOC_FAST_LOOP_isr(void) {
	...
	if (Mode_Sequence_Moteur == MODE_FCT_NORM) {
		...
		gd3000_Update_DutyCycle_Cmd_PWM_ABC(Cmd_DutyA,Cmd_DutyB,Cmd_DutyC,PWMPeriod,DeadTime_PWM,Align_PWM);
