...
//variables liées à la commande PWM et la mesure de courant synchronisée avec la PWM.
...
extern uint8_t Align_PWM;
extern uint32_t Delay_Trig_ADC_us; //délai en µs entre le trigger issu de la PWM (dépassement VAL1 pour déclencher l'acquisition
...
int main(void)
{
	...
	/***************************************************************/
	//arguments =
	//MOTEUR_REEL ou MOTEUR_VIRTUEL
	//SVM_ON ou SVM_OFF.
	//la valeur du dead time en nombre de cycle d'horloge module
	//EDGE_ALIGN ou CENTER_ALIGN
	init_Cmd_Moteur_Variables(MOTEUR_VIRTUEL,SVM_OFF,100,CENTER_ALIGN);
	...
	Current_sensor_OK = Current_sensor_Config_Init(Delay_Trig_ADC_us);
	...
	if (SIUL2.DISR0.B.EIF30 == 1) {
		...
		if (Mode == MODE_OFF) {
			...
			if (appui_SW1 == 0) {
				...
				gd3000_Update_DutyCycle_Cmd_PWM_ABC(500,500,500,PWMPeriod,DeadTime_PWM,Align_PWM);
